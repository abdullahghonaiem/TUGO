...
class BusyIndicator extends StatelessWidget {
  ...
  factory BusyIndicator({Color? color, double size = 20.0}) =>
      BusyIndicator._(SpinKitThreeBounce(color: color, size: size));

  factory BusyIndicator.accent(BuildContext context, {double size = 20.0}) {
    return BusyIndicator._(SpinKitThreeBounce(
        color: Theme.of(context).colorScheme.secondary, size: size));
  }

  factory BusyIndicator.round(
      {Color color = const Color(0xFF2F333A), double size = 50.0}) {
