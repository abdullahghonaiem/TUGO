...
//  static Completer _loader;
  ...
  static Future<T?> showLoader<T>(
    ...
    showModalBottomSheet(
      ...
      enableDrag: false,
      builder: (context) =>
          AppBottomSheetLoader(title: title, content: content),
    );
      ...
      futures.add(
        Future.wait(
          [
            ...
            value.firstWhere((element) => element != null, orElse: () => null)),
      );
...
class AppBottomSheetLoader<T> extends StatelessWidget {
  ...
  const AppBottomSheetLoader({Key? key, required this.title, this.content})
      : super(key: key);
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            BusyIndicator.round(),
            SizedBox(
                height: title.isNotEmpty || (content?.isNotEmpty ?? false)
                    ? defaultPadding * 1.5
                    : 0),
            if (title.isNotEmpty)
              Text(
