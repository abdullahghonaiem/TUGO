...
import 'CustomersettingProfileScreen.dart';
...
class _CustomerOTPState extends State<CustomerOTP> {
  ...
  Future<void> verifyOTP() async {
    try {
      ...
      bool codesMatch =
          await VerificationStorage().compareVerificationCode(enteredCode);
      ...
      if (codesMatch) {
        ...
        String phoneNumberWithoutCode =
            widget.phoneNumber.replaceAll(RegExp(r'^\+'), '');
        int numRatings = 0;
        int averageRating = 5;
        ...
        Navigator.pushReplacement<void, void>(
          ...
          MaterialPageRoute<void>(
            builder: (BuildContext context) => CustomerSettingProfile(),
          ),
        );
      } else {
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              title: Text("Error"),
              content: Text(
                  "The entered OTP does not match. Please check your OTP and try again."),
              actions: <Widget>[
                TextButton(
                  onPressed: () {
                                      ...
                                      child: TextFormField(
                                        onChanged: (value) {
                                          if (value.isNotEmpty) {
                                            ...
                                            FocusScope.of(context)
                                                .previousFocus(); // Move focus to the previous field
                                          }
                                        },
